...
%             save("E:\动态偏好\transfer-dynamics\code\PlatEMO-master\PlatEMO-master\PlatEMO\IGD\KTSDF5"+".mat","Score_IGD");
            score = mean(Scores);
            %独立重复实验，保留每一次的MIGD值
            fileID = fopen('E:\动态偏好\Knowledge-Transfer-GMM\Code\PlatEMO-master\PlatEMO\20IGD-RDF\IGD_RDF5.txt', 'a'); % 'a' 表示以追加模式打开文件
            fprintf(fileID, '%0.5f\n', score);
            fclose(fileID);
        end
%         %% Calculate the metric value
