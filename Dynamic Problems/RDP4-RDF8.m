...
%             save("E:\动态偏好\transfer-dynamics\code\PlatEMO-master\PlatEMO-master\PlatEMO\IGD\KTSDF8"+".mat","Score_IGD");
            score = mean(Scores);
            fileID = fopen('E:\动态偏好\Knowledge-Transfer-GMM\Code\PlatEMO-master\PlatEMO\20IGD-RDF\IGD_RDF8.txt', 'a'); % 'a' 表示以追加模式打开文件
            fprintf(fileID, '%0.5f\n', score);
            fclose(fileID);
        end
        %% Calculate the metric value
...
%         end
        ...
        function DrawObj(obj,Population)
            ...
            for i = 1 : length(change)-1
                ...
                Draw(obj.Optimums{find(G(change(i)+1)==allG,1),2}+(i-1)*0.5,'-','LineWidth',1,'Color',color);
