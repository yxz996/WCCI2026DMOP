...
%             end
        ...
        function DrawObj(obj,Population)
            ...
            G      = sin(0.5*pi*t);
            G      = round(G*1e6)/1e6;
            change = [0;find(G(1:end-1)~=G(2:end));length(G)];%当t没变的时候，change就是[0;100],当变了的时候，length(change)就大于2了(就是为了后面画出每次变化后对应的个体)
            ...
            for i = 1 : length(change)-1
                ...
                Draw(obj.PF+(i-1)*0.1,'-','LineWidth',1,'Color',color);%PF不变，故truePF要加一定步长来区分
            end
        end
    end
end